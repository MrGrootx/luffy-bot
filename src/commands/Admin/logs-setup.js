...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) return await interaction.reply({ content: 'You **do not** have the permission to do that!', ephemeral: true});
        ...
        switch (sub) {
            ...
            if (data) return await interaction.reply({ content: `You have **already** set up the logging system! \n> To disable Do **/logs disable**`, ephemeral: true});
                ...
                .setTimestamp()
                .addFields({ name: `Logging was Enabled`, value: `Your logging system has been set up successfuly...`})
            ...
            if (!data) return await interaction.reply({ content: `You have **not** set up the logging system! \n> To enable Do **/logs setup**.`, ephemeral: true});
            else {
 
                const disableembed = new EmbedBuilder()
                .setColor('NotQuiteBlack')
                .setTimestamp()
                .addFields({ name: `Logging was Disabled`, value: `Your logging system has been disabled successfuly...`})
